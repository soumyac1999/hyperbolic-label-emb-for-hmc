...
from avg_prec import ndcg_score, dcg_score,spearman_rank_correlation
...
x=0
for i in range(n_labels):
	x += spearman_rank_correlation(hops[:,i], pred[:,i].numpy())
...
mb = torch.load(f'rcv1_exp/poin_0.01_textcnn/14')['label_embs']
emb = emb/(1+torch.sqrt(1+emb.norm(dim=-1, keepdim=True)**2))
emb = emb.detach().cpu()
